...
archNovedades=($NOVEDIR/*.txt)
cantArchProc=${#archNovedades[@]}
./Logging Listener "Cantidad de archivos de novedades: $cantArchProc" INFO
	...
	do
		...
		if [ "$t_compras" != 0 ] && [ "$t_precios" != 0 ] ; then
			...
			if [ "$anioIngr" -gt "$anioAct" ] ; then
				./Logging Listener "Archivo: $nombre rechazado porque la fecha es invalida." ERR
				./Mover $arch $RECHDIR Listener	
				continue		
				#fecha invalida
